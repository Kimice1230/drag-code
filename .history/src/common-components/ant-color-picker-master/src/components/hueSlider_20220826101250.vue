...
import draggable from "@/common-components/colorPicker/src/lib/draggable";

import type { PropType } from "vue";
import type Color from "@/common-components/colorPicker/src/lib/color";
import type { Nullable } from "@/common-components/colorPicker/src/type/types";
	...
	setup(props) {
		...
		function handleDrag(event) {
			...
			if (!props.vertical) {
				...
				left = Math.max(
					(thumb.value as HTMLElement).offsetWidth / 2,
					left
				);
				...
				hue = Math.round(
					((left - (thumb.value as HTMLElement).offsetWidth / 2) /
						(rect.width -
							(thumb.value as HTMLElement).offsetWidth)) *
						360
				);
			} else {
				...
				top = Math.max(
					(thumb.value as HTMLElement).offsetHeight / 2,
					top
				);
				hue = Math.round(
					((top - (thumb.value as HTMLElement).offsetHeight / 2) /
						(rect.height -
							(thumb.value as HTMLElement).offsetHeight)) *
						360
				);
			}
